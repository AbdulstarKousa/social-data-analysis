...
crimes_data = pd.read_csv(crimes_path, usecols=['Category', 'Date', 'Time', 'PdDistrict', 'Y'])
...
weather_data = pd.read_csv(weather_url, usecols=['date','temperature']) 
...
""" check if all entries has temperature """
temperature_check    = sum(pd.isnull(Data['temperature']))
print (f'number of temperature missing values = {temperature_check}')    
...
validation_size = 0.40  # 17161 samples to train on  and 11441 samples to validate (unseen data)
...
"""
> Accuracy:      0.624
> random_best_params_:   {'n_estimators': 1000, 
                          'min_samples_leaf': 75}
"""

# prepare final model (with GridSearchCV best hyperparameters)
model = Pipeline([('preprocessor', preprocessor),('rf',RandomForestClassifier(n_estimators= 1000, min_samples_leaf = 75, max_features = 'sqrt', criterion = 'entropy'))])
...
>
    ...
    BURGLARY       0.63      0.63      0.63      5721
       FRAUD       0.63      0.64      0.63      5720

    accuracy                           0.63     11441
   macro avg       0.63      0.63      0.63     11441
weighted avg       0.63      0.63      0.63     11441
"""
...
>
Hour                0.114 +/- 0.003
PdDistrict          0.041 +/- 0.002
temperature         0.018 +/- 0.001
"""
...
""" 4 
Hour                0.120 +/- 0.004
PdDistrict          0.036 +/- 0.003
temperature         0.018 +/- 0.001
wind_direction      0.016 +/- 0.002
"""
